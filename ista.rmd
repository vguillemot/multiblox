...
#   G <- neg_step(X, y, t, beta, alpha)
#   return(ifelse(R(X, y, beta - t*G) <= R(X, y, beta) - t*t(grad(X,y,beta)) %*% G + (t/2)*sum(G)**2, TRUE, FALSE)) 
# }
# step_line_search <- function(X, y, beta, t=10, tau=0.5, alpha){
...
(res1$k)
(res2$k)
knitr::kable(res<-data.frame("ISTA"=res1$beta, "ISTA adaptatif" = res2$beta, "$\\beta^*$" = betastar,check.names = F))
```
