...
#     x1_acp=prcomp(simu$X[[1]])
#     x2_acp=prcomp(simu$X[[2]])
#     print(cor(x1_acp$x[, 1],x2_acp$x[, 1]))
#     pdf("2Bbis_logistic_simulated_data_withCGH_PCA.pdf", h=8, w=8)
#     plot(x1_acp$x[, 1]~x2_acp$x[, 1], pch=(simu$y)+2, xlab="PC1 block2", ylab="PC1 block1", main="2bis block-simulated with CGH data first PC")
#     dev.off() 
# 
# ## visualisation des données par PCA
# x1_acp=prcomp(simu4$X[[1]])
# x2_acp=prcomp(simu4$X[[2]])
# x3_acp=prcomp(simu4$X[[3]])
# pdf("3B_logistic_simulated_data_visu_PCA.pdf", h=8, w=8)
# plot(x1_acp$x[, 1]~x2_acp$x[, 1], pch=(simu4$y)+2, xlab="PC1 block2", ylab="PC1 block1", main="3 block-simulated data first PC")
# plot(x1_acp$x[, 1]~x3_acp$x[, 1], pch=(simu4$y)+2, xlab="PC1 block3", ylab="PC1 block1", main="3 block-simulated data first PC")
# plot(x2_acp$x[, 1]~x3_acp$x[, 1], pch=(simu4$y)+2, xlab="PC1 block3", ylab="PC1 block2", main="3 block-simulated data first PC")
# dev.off() 
