istacox.predict <- function(model, x, y, lambda, type=c("spll", "deviance", "pi", "all")){
  ...
  if(type=="spll"){
    ...
    print("deviance")
    dev <- sparse.partial.loglik(model=model, newdata=x, newy=y, lambda=lambda)
    print(paste("Loglik du modele : ", dev, sep=""))
    dev_null <- sparse.partial.loglik(model=null_model, newdata=x, newy=y, lambda=lambda)
    print(paste("Loglik du modele nul : ", dev_null, sep=""))
